...
function loadStatsPage(){
	...
	for (var i = 2; i < x.rows.length; i++){
		games++;
		if (parseInt(x.rows[i].cells[2].innerHTML) >= parseInt(x.rows[i].cells[3].innerHTML)){
			buff_wins++;
			x.rows[i].cells[4].innerHTML = "CU Boulder";
		}
		else{
			x.rows[i].cells[4].innerHTML = x.rows[i].cells[1].innerHTML;
		}
	}
	document.getElementsByTagName("table")[1].rows[1].cells[0].innerHTML = buff_wins;
	document.getElementsByTagName("table")[1].rows[1].cells[1].innerHTML = (games-buff_wins);
...
function loadPlayersPage(){
	...
	for(var i=0; i<players.length; i++){
		...
		// set attribute onclick to call switchPlayers() function
		x.setAttribute("onclick", "switchPlayers("+i.toString()+");");
